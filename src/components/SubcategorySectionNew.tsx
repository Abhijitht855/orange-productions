...
//   return (
//     <section id={encodeURIComponent(title)} className="mb-10">
...
// const SubcategorySection: React.FC<SubcategorySectionProps> = ({ title, items }) => {
//   return (
//     <section id={slugify(title)} className="mb-10">
//       <h2 className="text-lg font-bold text-gray-800 border-b border-gray-300 pb-2 mb-4">
//         {title}
//       </h2>
...
const SubcategorySection: React.FC<SubcategorySectionProps> = ({ title, items }) => {
  return (
    <section id={slugify(title)} className="mb-10">
      ...
      {/* Grid of category cards with staggered animation */}
      <motion.div
        className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4"
        initial="hidden"
        animate="visible"
      ...
      >
        {items.map((item) => (
          <motion.div
            key={item.name} // Use item.name as key instead of index for better React reconciliation
            variants={{
